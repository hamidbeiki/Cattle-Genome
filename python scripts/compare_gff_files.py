...
lists=list(same_trs_spliced.values())
...
fl=flatten_nested_list(lists)
fl=list(dict.fromkeys(fl))

valids=[]
for i in fl:
    if i.find('Isoseq')==-1:#or Nanopoore or Trinity
        a=i.split('.')
        tr='PB.'+a[1]+'.'+a[2]
        valids.append(tr)

df=pd.DataFrame({'transcripts':valids})

df.to_csv(tis + '_validated_transcripts.txt',index = None, header=False,sep="\t",quoting=csv.QUOTE_NONE)
#load
#same_trs_spliced = np.load('same_trs_spliced.npy',allow_pickle='TRUE').item()
#same_trs_spliced_with_retained = np.load('same_trs_spliced_with_retained.npy',allow_pickle='TRUE').item()
