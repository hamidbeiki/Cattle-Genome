...
def pairs(lst):
    i = iter(lst)
    first = prev = item = i.next()# use i.next() in python 2.7
    for item in i:
        yield prev, item
        prev = item
    yield item, first
...
"""
isize=10#size of each item (number of itersections rows passed to each cpu) in items
p = Pool()
items=get_items_fromLists(mergable_same_trs_spliced) 
start = time.time()
processed_values= p.map(get_redundancy_parallel, get_chunks(items, 100))
p.close()
end = time.time()
print('total time (s)= ' + str(end-start)) 
redundants=[]
...
""" The following section doesn't need to be included in combine gff files program
    and just required when the goal is to remove within tissue redundancy"""
